...
function search_tweets ($search_tweets, $get_query, $connection) {
	...
	//Capture URL
	if ($_GET['do'] == 'getNextTrack') {
		$p['q'] = '(soundcloud OR rd.io/x) from:' . $get_query . ' filter:links';
	} else {
		$p['q'] = 'from:' . $get_query . ' filter:links';
	}
	//$p['count'] = 10;
	...
	foreach($result->statuses as $s) 
	{
		foreach($s->entities->urls as $url) 
		{
			...
			if ((stristr($urlLocation, 'soundcloud.com') !== FALSE) || (stristr($urlLocation, 'snd.sc') !== FALSE)) {
				...
				$package['id'] = $s->id;
				$package['src'] = 'soundcloud';
				array_push($array, $package);
				break 2;
			} else if ((stristr($urlLocation, 'rd.io') !== FALSE) || (stristr($urlLocation, 'rdio.com') !== FALSE)) {
				$package = array();
				$package['user'] = $get_query;
				$package['track_url'] = $url->expanded_url;
				$package['id'] = $s->id;
...
function getRdioCurl($url) {
	$ch = curl_init(); 

        // set url 
        curl_setopt($ch, CURLOPT_URL, 'http://www.rdio.com/api/oembed/?format=json&url=' . $url); 
...
switch ($what) {
	...
	case 'searchTrack':
		...
		break;
	case 'getRdioCurl':
		echo json_encode(getRdioCurl($_GET[q]));
		break;
	case 'getEmbed':
